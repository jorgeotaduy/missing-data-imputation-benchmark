...
| Imputation Method | F1-score (mean) | ROC AUC (mean) | PR AUC (mean) |
|---|---|---|---|
| GAIN | **0.402** | **0.878** | **0.463** |
| MissForest | **0.402** | 0.870 | 0.449 |
| Median | 0.391 | 0.862 | 0.429 |
| Mean | 0.390 | 0.861 | 0.429 |
| kNN | 0.380 | 0.859 | 0.426 |
| MICE | 0.376 | 0.872 | 0.443 |

Selected findings:
- **GAIN + Random Forest** and **MissForest + Random Forest** are the best-performing combinations across all metrics.
- **Simple methods (mean, median) are surprisingly competitive**, especially paired with Random Forest.
- **MCAR is the most damaging mechanism** — performance degrades progressively with missingness rate.
- **MNAR yields the best results** among simulated mechanisms, likely due to the structured and predictable nature of the simulated extremes.
- **SMOTE improves F1-score by ~5.8 points** on average, with a minor trade-off in PR AUC.
- The **choice of classifier has more impact than the choice of imputer**, as long as a reasonable imputation method is used.
